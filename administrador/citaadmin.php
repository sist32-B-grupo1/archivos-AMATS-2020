  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
	 ...
	 <div class="modal-header">
      ...
      <form method ="post">
        <!-- modificar cita -->
        <button type="button" class="sasei" data-toggle="modal" data-target="#exampleModal10" data-whatever="@fat" name="modifcars">modificar Cita</button>
        <!-- eliminar cita -->
...
<!-- inicio de modal modificar citas -->
<div class="modal fade" id="exampleModal10" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-xl">
    ...
    <tr>
            ...
            <?php 
               ...
               while ($columnac = mysqli_fetch_array( $resultados ))
                 { 
                    echo "<tr>";
                    echo "<td  valing=top style = vertical-align:text-top;>" ;
                    echo "<input type=radio name=modificarc value=" .$columnac['idcitas'] ."checked >";
                    echo "</td>";
                    echo "<td>" .$columnac['idcitas'] ."</td>"; 
                    echo "<td>" . $columnac['fecha'] ."</td><td>" . $columnac['hora'] ."</td>";
      ...
      <div class="modal-footer">
	  <button type="button" class="saa" data-dismiss="modal">Cerrar</button>
    <button type="button" class="sa" data-toggle="modal" data-target="#exampleModal11" data-whatever="@fat" name="modifcars">seleccionar Cita</button>
      </div>
  ...
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
     ...
     ?>
	   <div class="modal-body">
        <form method="post" action="citaadmin.php">
          ...
          <div class="form-group">
            ...
            <input type="date" class="form-control"  name="fechanuevo">
          </div>
          <div class="form-group">
            <label for="recipient-name" class="col-form-label">Hora</label>
            <input type="time" class="form-control"  name="horanuevo">
          </div>
          <div class="form-group">
            <label for="message-text" class="col-form-label">Servicio</label>
            <button type="button" class="s" data-toggle="modal" data-target="#exampleModal16" data-whatever="@fat">Agregar Servicios</button>   
          </div>
      ...
      <div class="modal-footer">
        <button type="button" class="saa" data-dismiss="modal">Cerrar</button>
        <input type="submit" value="modificar" name="modificarci" class="s">
...
<div class="modal fade" id="exampleModal16" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-xl">
    ...
    <table class="table">  
            <?php 
               $consultac = 'SELECT * from servicios ';
               $resultadoc = mysqli_query($conexion,$consultac) ;//or die ( "Algo ha ido mal en la consulta a la base de datos");
               $contador = 0;
               echo "<tr>";
               while ($columnac = mysqli_fetch_array( $resultadoc ))
                 { 
                    echo "<td  valing=top style = vertical-align:text-top;>" ;
                    echo "<input type=checkbox name=cla[] value=" .$columnac['idservicios'] .">"  .$columnac['nombreservicios'] .$serviciotn=$columnac['idtipo'];
                    echo "</td>";
                    $contador++;
                    if ($contador == 5) {
                      echo "</tr>";
...
<?php
 if (isset($_POST["modificarci"])) {
 
   $modificarc = $_POST["modificarc"];
   $fechan = $_POST["fechanuevo"];
   $horan = $_POST["horanuevo"];
   $servicion = $_REQUEST["cla"];
   $nn= count ($servicion);
   ...
   if (isset($servicion))
   {
     if (isset($nn)) {
       ...
       for($l=0; $l<$nn; $l++){
   $modificarcit =  "UPDATE citas SET fecha = '$fechan', hora = '$horan', idservicio = '$servicion[$l]', idtipo = '$serviciotn' WHERE idcitas = '$modificarc'";
   $modificarcii = mysqli_query($conexion,$modificarcit);
       }
      }
    }
 }
    ...
    if (isset($servicio))
    {
      if (isset($n)) {
        ...
        for($i=0; $i<$n; $i++){
          ...
          $insertarcita =  "INSERT INTO citas (idcliente,idservicio,idtipo,hora,fecha,direccion,idadmin,descripcioncita,telefono) VALUES ('$idcliente', '$servicio[$i]', '$serviciot',  '$hora', '$fecha', '$direccion','$idadmin','$comentario','$telefono')";
